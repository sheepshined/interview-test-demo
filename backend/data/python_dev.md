...
### A: 不可变对象：int、float、str、tuple、frozenset——内容不可修改，修改会创建新对象。可变对象：list、dict、set——内容可原地修改。函数传参是引用传递（传对象的引用），不是值传递也不是引用传递。不可变对象在函数内重新赋值不影响外部（因为创建了新对象），可变对象在函数内修改内容会影响外部。
### S:
- 正确列举了可变和不可变对象的类型
